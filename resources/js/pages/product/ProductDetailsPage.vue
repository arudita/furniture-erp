                ...
                <div class="flex flex-col lg:flex-row gap-6">
                    <div class="flex-1 max-w-xl space-y-6">
                        ...
                        <div class="grid gap-2">
                            ...
                            <Input id="unit_price" name="unit_price" v-model="display_unit_price" @input="handleInputTotal($event)" @blur="formatDisplayTotal()" @focus="unformatDisplayTotal()" type="text" maxlength="16" class="mt-1 block w-full" :class="errors?.unit_price ? 'border-red-500': ''" placeholder="Unit Price" required />
                    ...
                    <div class="flex-1 max-w-xl space-y-6">
                        <div class="grid gap-2">
                            ...
                            <Label for="image_upload" :class="{ 'dragging': isDragging }" @dragover.prevent @dragenter="handleDragOver" @drop="handleDrop($event)" class="flex flex-col items-center file:text-foreground placeholder:text-muted-foreground selection:bg-primary selection:text-primary-foreground dark:bg-input/30 border-input rounded-md border bg-transparent p-6 text-base shadow-xs transition-[color,box-shadow] outline-none file:border-0 file:bg-transparent file:text-sm file:font-medium disabled:pointer-events-none disabled:cursor-not-allowed disabled:opacity-50 md:text-sm focus-visible:border-ring focus-visible:ring-ring/50 focus-visible:ring-[3px] aria-invalid:ring-destructive/20 dark:aria-invalid:ring-destructive/40 aria-invalid:border-destructive mt-1 w-full">
                                ...
                                <Input
                                    type="file" id="image_upload" ref="imageFileInput" multiple @change="handleImageUpload" :disabled="maxImagesReached"
...
const selectCategory = (selectedValue: number) => {
    selectedCategoryId.value = selectedValue;
    form_product.category_id = selectedValue;
...
// Handle Input Total
const handleInputTotal = (e: any) => {
    const input = e.target.value.replace(/\D/g, '');

    form_product.unit_price = input === '' ? 0 : parseInt(input, 10);

    formatDisplayTotal();
}

const formatDisplayTotal = () => {
    display_unit_price.value = 'Rp' + form_product.unit_price.toString().replace(/\B(?=(\d{3})+(?!\d))/g, '.');
};

const unformatDisplayTotal = () => {
    display_unit_price.value = form_product.unit_price.toString();
};
const submit = () => {
